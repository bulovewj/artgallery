...
:root {
    ...
    --warm-white: #FAF7F2;
    --espresso: #2C1810;
    --latte: #8B6F5E;import streamlit as st
from utils import load_paintings, get_contact_info
...
.painting-card:hover .painting-img-container img {
...
with col3:
    if st.button("✉️  연락하기", key="nav_contact", use_container_width=True):
        ...
        st.rerun()
with col4:
    if st.button("🔒  관리자", key="nav_admin", use_container_width=True):
        st.session_state.page = 'admin'
        st.rerun()
